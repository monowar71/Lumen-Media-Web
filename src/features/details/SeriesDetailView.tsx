...
export function SeriesDetailView({ series }: { series: SeriesDetail }) {
  ...
  // Manual pick keyed by series id — switching series drops the override without an effect.
  const [seasonOverrideBySeries, setSeasonOverrideBySeries] = useState<Record<string, string>>({});
              ...
              <div className="no-scrollbar flex gap-2 overflow-x-auto pb-1">
                {seasons.map((s) => (
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      setSeasonOverrideBySeries((prev) => ({ ...prev, [series.id]: s.id }))
                    }
                    className={cn(
